...
[chasings_per_day]=plot_tube_chasings_n_per_day(data_dir,save_dir,cohortsTbl);

% save chasing data
writetable([cohortsTbl(:,1),array2table(chasings_per_day)],fullfile(save_dir,'chasings_per_day.csv'));
writetable([cohortsTbl(:,1),array2table([nanmean(chasings_per_day,2),nanmedian(chasings_per_day,2)],'VariableNames',{'mean','median'})],fullfile(save_dir,'chasings_per_day.csv'));
disp(['!!! Check median/mean data in ' save_dir ' and decide which cohorts to use !!!'])


% Plot of tube chasings
plot_tube_chasings_daycycle(data_dir,save_dir,cohortsTbl);

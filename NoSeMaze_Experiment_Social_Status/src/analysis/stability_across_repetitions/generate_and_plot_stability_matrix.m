function generate_and_plot_stability_matrix(data_table, X_metrics, Y_metrics, day_ranges, output_dir, method, LME_covariates)
...
for day_idx = 1:numel(day_ranges)
    ...
    corr_matrix = nan(num_X_metrics, num_Y_metrics);
    pval_matrix = nan(num_X_metrics, num_Y_metrics);
    corr_matrix_partial = nan(num_X_metrics, num_Y_metrics);
    pval_matrix_partial = nan(num_X_metrics, num_Y_metrics);

    % Check if LME_covariates exists
    use_covariate = ~isempty(LME_covariates) && ~isempty(LME_covariates{1}) && isfield(current_data, LME_covariates{1});
    ...
    for i = 1:num_X_metrics
        for j = 1:num_Y_metrics
            ...
            rho = nan; pval = nan; rhopartial = nan; pvalpartial = nan;

            if isnumeric(xData) && isnumeric(yData)
                [rho, pval] = corr(xData, yData, 'Rows', 'complete', 'Type', method);
                if use_covariate
                    covariate_vec = double(strcmp(current_data.(LME_covariates{1}), 'WT'));
                    [rhopartial, pvalpartial] = partialcorr(xData, yData, covariate_vec, 'Rows', 'complete', 'Type', method);
                else
                    rhopartial = rho;
                    pvalpartial = pval;
                end
            elseif length(unique(xData)) == 2
                xData_unique = unique(xData);  % <-- assign to variable first
                xData_bin = strcmp(xData, xData_unique{1});
                [rho, pval] = corr(xData_bin, yData, 'Rows', 'complete', 'Type', method);
                if use_covariate
                    covariate_vec = double(strcmp(current_data.(LME_covariates{1}), 'WT'));
                    [rhopartial, pvalpartial] = corr(xData_bin, yData, covariate_vec, 'Rows', 'complete', 'Type', method);
                else
                    rhopartial = rho;
                    pvalpartial = pval;
                end
    ...
    for matrix_idx=1:2
        ...
        for i = 1:num_X_metrics
            for j = 1:num_Y_metrics
                if current_pval_matrix(i, j) < pBonferroni
    ...
    % T = T(contains(T.measure,{'switch','hit'}) & ~contains(T.measure,'cuberoot'),:);
    T = T(~contains(T.measure,'cuberoot'),:);

    % Create bar chart
    fig = figure('Visible','on');
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.7, 0.4]);

    % barchart
    b = bar(T.correlation, 'FaceColor', 'flat', 'EdgeColor', 'none'); % Light blue bars
    ...
    xticklabels(T.measure);
    ylabel('Correlation Coefficient (r)');
    title('Correlation Between Round 1 and Round 2');
    ylim([0 1]); % Set y-axis range for correlation
    grid on;
    ...
    clim([-1 1]); % Set colorbar limits to match correlation range

    %% **Add significance markers**
    hold on;
    for i = 1:length(T.correlation)
        % Define marker based on p-value
        if T.pval(i) < 0.001
            significance_marker = '***';
        elseif T.pval(i) < 0.01
            significance_marker = '**';
        elseif T.pval(i) < 0.05
            significance_marker = '*';
        else
            significance_marker = ''; % No marker for non-significant values
        end

        % Only plot if significant
        if ~isempty(significance_marker)
            text(i, T.correlation(i) + 0.05, significance_marker, ...
    ...
    % SourceData
    writetable(T, fullfile(output_dir, sprintf('SourceData_%s_BarPlot.csv', range_name)));


    % Save the heatmap
    [annot, srcInfo] = docDataSrc(fig,fullfile(output_dir),mfilename('fullpath'),logical(1));
    exportgraphics(fig, fullfile(output_dir, sprintf('BarPlot_%s.pdf', range_name)));
    close(fig);
end
end

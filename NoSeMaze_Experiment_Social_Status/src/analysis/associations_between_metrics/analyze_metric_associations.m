function analyze_metric_associations(data_table, X_metrics, Y_metric, day_ranges, output_dir, LME_covariates)
% Analyze associations between X_metrics and a specified Y_metric.
%
% Parameters:
% data_table - Summary data table
% Y_metric - Y_metric to analyze (e.g., 'DSz_Competition')
% X_metrics - Cell array of X_metric names (e.g., {'weight_pre', 'genotype'})
...
for day_idx = 1:numel(day_ranges)
    ...
    yData = current_data.(Y_metric);
    % Initialize LME Table
    if ~isempty(LME_covariates) && ~strcmp(LME_covariates{1}, '')
        % Initialize with Animal ID, xData, and yData
        LME_DataTable = table(current_data.Mouse_RFID, yData, 'VariableNames', {'Mouse_RFID', Y_metric});
    ...
    % Add X_metrics to the table
    for X_metric_idx = 1:numel(X_metrics)
        X_metric_name = X_metrics{X_metric_idx};
        LME_DataTable.(X_metric_name) = current_data.(X_metric_name);
    end
   
    % Display number of entries for debugging
    fprintf('Day range: %s | Y_metric: %s | Entries: %d\n', range_name, Y_metric, height(LME_DataTable));
    ...
    for X_metric_idx = 1:numel(X_metrics)
        ...
        xData = LME_DataTable.(X_metric_name);
        yData = LME_DataTable.(Y_metric);
        
        %% Plot Correlation
        fig = figure('visible','off');
        
        % Correlation coefficients
        if length(unique(LME_DataTable.(X_metric_name)))~=2
            [rho, pval] = corr(xData, yData, 'type', 'Pearson', 'rows','complete');
            [rho_sp, pval_sp] = corr(xData, yData, 'type', 'Spearman', 'rows','complete');
        end
        % Plot scatter plot or notBoxPlot for binary vs continuous X_metrics
        if length(unique(LME_DataTable.(X_metric_name)))==2
            % notBoxPlot
            nb = notBoxPlot(LME_DataTable.(Y_metric), categorical(LME_DataTable.(X_metric_name)));
            % color definitions
            for ib = 1:length(nb)
        ...
        if contains(Y_metric,'Rank') || contains(Y_metric,'N_') || contains(Y_metric,'Fraction_')
            ax.YLim(1)=0;
        elseif contains(Y_metric,'cuberoot_Fraction_') || contains(Y_metric,'Fraction_')
            ax.YLim=[0,1]; 
        else
            if min(yData)<0
                ax.YLim=([1.1*min(yData) 1.1*max(yData)]);
            else
                ax.YLim=([0.9*min(yData) 1.1*max(yData)]);
            end
        end
        
        if length(unique(LME_DataTable.(X_metric_name)))~=2
            if contains(X_metric_name,'Rank')  || contains(X_metric_name,'N_') || contains(X_metric_name,'Fraction_')
                ax.XLim(1)=0;
            else
                if min(xData)<0
        ...
        % Customize plot
        ax.XLabel.String = X_metric_name;
        ax.XLabel.Interpreter = 'none';
        ax.YLabel.String = Y_metric;
        ax.YLabel.Interpreter = 'none';
        if length(unique(LME_DataTable.(X_metric_name)))~=2
            title({sprintf('%s vs %s (%s)', Y_metric, X_metric_name, range_name), ...
                sprintf('Pearson R = %.2f, p = %.3f', rho, pval), ...
                sprintf('Spearman R = %.2f, p = %.3f', rho_sp, pval_sp)},'Interpreter','none');
        else
            title({sprintf('%s vs %s (%s)', Y_metric, X_metric_name, range_name)},'Interpreter','none')
        end
        
        % Save Plot
        [annot, srcInfo] = docDataSrc(fig,fullfile(output_dir),mfilename('fullpath'),logical(1));
        if length(unique(LME_DataTable.(X_metric_name)))==2
            exportgraphics(fig, fullfile(output_dir, sprintf('NotBoxPlot_%s_vs_%s_%s.pdf', Y_metric, X_metric_name, range_name)));
        else
            exportgraphics(fig, fullfile(output_dir, sprintf('Scatterplot_%s_vs_%s_%s.pdf', Y_metric, X_metric_name, range_name)));
        end
        close(fig);
        ...
        dependent_variable = Y_metric; % Target variable for the LME
        ...
        if ~isempty(LME_covariates) && ~strcmp(LME_covariates{1}, '')
            predictors = [X_metric_name,LME_covariates];
        else
            predictors = X_metric_name;
        end
        ...
        try
            ...
            warning('LME failed for %s in %s: %s', Y_metric, range_name, ME.message);
            continue;
        end
        ...
        if ~isempty(LME_covariates) && ~strcmp(LME_covariates{1}, '')
            writetable(fixedEffectsTable, fullfile(output_dir, sprintf('LME_results_%s_vs_%s_cov_%s_%s.csv', Y_metric, X_metric_name, LME_covariates_names, range_name)));
            % Save source data
            writetable(LME_DataTable(:,[grouping_variable, dependent_variable, predictors]), fullfile(output_dir, sprintf('SourceData_%s_vs_%s_cov_%s_%s.csv', Y_metric, X_metric_name, LME_covariates_names, range_name)));
        else
            writetable(fixedEffectsTable, fullfile(output_dir, sprintf('LME_results_%s_vs_%s_%s.csv', Y_metric, X_metric_name, range_name)));
            % Save source data
            writetable(LME_DataTable(:,[{grouping_variable}, {dependent_variable}, predictors(:)']), fullfile(output_dir, sprintf('SourceData_%s_vs_%s_%s.csv', Y_metric, X_metric_name, range_name)));
        end
    end
end

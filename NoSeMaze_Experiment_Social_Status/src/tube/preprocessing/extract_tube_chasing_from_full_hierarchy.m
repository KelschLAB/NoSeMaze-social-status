function full_hierarchy = extract_tube_chasing_from_full_hierarchy(full_hierarchy,ops)
...
ID_vector = vertcat(full_hierarchy.ID);
% delete empty entries
ID_vector = ID_vector(~cellfun(@isempty, ID_vector));
all_IDs = unique(ID_vector);
filtered_detections(~contains(filtered_detections.Animal,all_IDs),:) = [];
...
for tt = 1:2
    ...
    % visualize temporal distribution of potential close following events
     if ops.plotter
        ...
        ylimits = ylim; line([ops.threshold_lag_at_detector,ops.threshold_lag_at_detector],[0,ylimits(2)]);
        set_fonts()
    ...
    if ops.plotter
        ...
        ylabel('count');
        xlim([0 3]);
        ylimits = ylim; line([ops.threshold_lag_through_tube,ops.threshold_lag_through_tube],[0,ylimits(2)]);
        set_fonts()
...
all_days = datestr((datetime(ops.start_date):datetime(ops.end_date))', 'yyyy-mm-dd');
...
for day_num = 1:size(all_days,1)
    ...
    if isempty(full_hierarchy(day_num).Data)       
        % Create a placeholder hierarchy_data for missing days
        full_hierarchy(day_num).match_matrix_chasing = [];
        full_hierarchy(day_num).match_info_chasing = {};
        
    elseif ~isempty(full_hierarchy(day_num).Data)   
    
        cur_chasings = chasing_index(ismember(filtered_detections.Day(chasing_index),cur_day));
        
        % initialize matrix
        full_hierarchy(day_num).match_matrix_chasing = zeros(size(full_hierarchy(day_num).match_matrix));
        full_hierarchy(day_num).match_info_chasing = cell(size(full_hierarchy(day_num).match_info));
        for ii = 1:numel(cur_chasings)
            loser = filtered_detections.Animal{cur_chasings(ii)}; % == being chased
            loser_id = find(contains(all_IDs,loser));
            winner = filtered_detections.Animal{cur_chasings(ii)+1}; % == chasing
            winner_id = find(contains(all_IDs,winner));
            assert(~strcmp(winner,loser));
            
            % parse
            full_hierarchy(day_num).match_matrix_chasing(winner_id, loser_id) = full_hierarchy(day_num).match_matrix_chasing(winner_id, loser_id)+1;
            num = full_hierarchy(day_num).match_matrix_chasing(winner_id, loser_id);
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).winner_entry_time = filtered_detections.Time_sec(cur_chasings(ii)+1);
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).loser_entry_time = filtered_detections.Time_sec(cur_chasings(ii));
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).winner_exit_time = filtered_detections.Time_sec(cur_chasings(ii)+3);
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).loser_exit_time = filtered_detections.Time_sec(cur_chasings(ii)+2);
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).winner_ID = winner;
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).loser_ID = loser;
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).winner_entry_Detector = filtered_detections.Detector(cur_chasings(ii)+1);
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).winner_exit_Detector = filtered_detections.Detector(cur_chasings(ii)+3);
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).loser_entry_Detector = filtered_detections.Detector(cur_chasings(ii));
            full_hierarchy(day_num).match_info_chasing{winner_id,loser_id}(num).loser_exit_Detector = filtered_detections.Detector(cur_chasings(ii)+2);
        end

function DS_data = compute_DS(hierarchy_data)

exclude_multiple_animals = 1;

if exclude_multiple_animals 
    % remove multiple animals events
        ...
        for j=1:size(hierarchy_data.match_matrix,2)
            if ~isempty(hierarchy_data.match_info{i,j})
                for n=1:size(hierarchy_data.match_info{i,j},2)
                    if hierarchy_data.match_info{i,j}(n).multiple_animals
...
for i=1:size(hierarchy_data.match_matrix,1)
    for j=1:size(hierarchy_data.match_matrix,2)
        int(i,j)=hierarchy_data.match_matrix(i,j)+hierarchy_data.match_matrix(j,i);
    end
end


P=hierarchy_data.match_matrix./int;
...
%% parse output
DS_data.DS = DS;
DS_data.ID = hierarchy_data.ID;
DS_data.DS_sorted = DSv;
DS_data.DS_sortedIndex = DSi;
DS_data.match_matrix = hierarchy_data.match_matrix;

function lick_params = compute_CSminus_detection_speed(lickport_data, lick_params)
    ...
    % Define the time window of interest (from 0.5s to 2.5s after odor onset)
    % This time window will be used to bin the lick timestamps for analysis.
    edges = 0.5:0.05:2.5;  % Creates edges: [0.5, 0.55, 0.6, ..., 2.5]
    ...
    for tr = 1:numel(cs_minus_trials_before_reversal)
        ...
        cs_minus_lick_count = cat(1, cs_minus_lick_count, ...
    ...
    psth = mean(cs_minus_lick_count) * 20;  % This is the average lick rate over the time window.
    ...
    lick_params.cs_minus_detection_speed = 0.1 + 0.05 * find(psth(3:end) < -0.5 + lick_params.baseline_rate_CSminus_mean_omitfirst, 1, 'first');

    % If the detection speed is not found (i.e., no threshold exceeded), set it to NaN
    if isempty(lick_params.cs_minus_detection_speed)
        lick_params.cs_minus_detection_speed = NaN;
    end
end
